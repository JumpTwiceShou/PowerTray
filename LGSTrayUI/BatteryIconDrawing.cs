...
namespace LGSTrayUI
{
    public static partial class BatteryIconDrawing
    {
        ...
        public static void DrawAlert(TaskbarIcon taskbarIcon, LogiDevice device)
        {
            ...
            g.DrawImage(deviceIcon, destRect, 0, 0, deviceIcon.Width, deviceIcon.Height, GraphicsUnit.Pixel, wrapMode);

            using Pen pen = new(Color.FromArgb(0xE8, 0x11, 0x23), Math.Max(2, ImageSize / 12));
            g.DrawEllipse(pen, 2, 2, ImageSize - 4, ImageSize - 4);
            ...
            g.DrawString("!", font, brush, ImageSize / 2, ImageSize / 2, new(StringFormatFlags.FitBlackBox)
            {
                LineAlignment = StringAlignment.Center,
                Alignment = StringAlignment.Center,
            });
